...
def get_gemma_vision_model_and_processor(
    model_name: str = "google/gemma-3-27b-it",
):
    """Loads and caches the Gemma 3 multimodal model and processor."""

    if Gemma3ForConditionalGeneration is None:
        raise RuntimeError(
            "Gemma3ForConditionalGeneration is unavailable; install transformers>=4.50.0"
        )

    cache_key_model = f"gemma3_{model_name}_model"
    cache_key_processor = f"gemma3_{model_name}_processor"

    if cache_key_model not in _loaded_models:
        logger.info(f"ModelLoader: Loading Gemma 3 model '{model_name}' on {DEVICE}...")
        try:
            processor = AutoProcessor.from_pretrained(model_name)
            dtype = torch.float16 if DEVICE == "cuda" else None
            model = Gemma3ForConditionalGeneration.from_pretrained(model_name, torch_dtype=dtype)
            model = model.to(DEVICE)
            model.eval()

            _loaded_models[cache_key_model] = model
            _loaded_models[cache_key_processor] = processor
            logger.info(f"ModelLoader: Gemma 3 model '{model_name}' loaded and cached successfully.")
        except Exception as e:
            logger.exception(f"ModelLoader: Failed to load Gemma 3 model '{model_name}'.")
            raise RuntimeError(f"Could not load Gemma 3 model '{model_name}': {e}") from e
    else:
        logger.debug(f"ModelLoader: Using cached Gemma 3 model '{model_name}'.")
        model = _loaded_models[cache_key_model]
        processor = _loaded_models[cache_key_processor]
